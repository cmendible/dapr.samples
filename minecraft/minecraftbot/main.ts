...
const sentimentUrl = `http://localhost:${daprPort}/v1.0/invoke/twitter-sentiment/method/sentiment`;
...
app.post('/tweets', (req, res) => {
    try {
        ...
        if (message !== undefined) {
            ...
            axios.post(sentimentUrl, "\"" + message + "\"", config)
                .then((res) => {
                    console.log(`Status: ${res.status} and score ${res.data}`);
                    bot.chat(`tweet: ${message} with score: ${res.data}`);
                }).catch((err) => {
                    // console.error(err);
                    // res.sendStatus(500);
                });
